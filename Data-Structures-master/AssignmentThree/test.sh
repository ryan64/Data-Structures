...
for casenum in `seq 1 1 $casecount`; do
    ...
    timeout -k $timeout $timeout ./$filename "operation=multiply;A=a$casenum.txt;B=b$casenum.txt;result=$casenum.out;storage=array;sort=nosort" 1>$casenum.stdout 2>$casenum.stderr 
    if [ $? -ne 0 ]; then
